...
void escribir(VentaTele Ventas,int TopeNombre,int TopeTelefonos){
    ofstream archivoB;
    int largo;
    largo = sizeof(VentaTele);

    archivoB.open("Prueba.dat",ios::app | ios::binary);
...
{
    VentaTele Ventas;
    /* escribir(Ventas,TOPEPROM,TOPETEL);  */
    leer(Ventas);
    return 0;
}
